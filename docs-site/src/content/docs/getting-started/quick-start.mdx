...
block-beta
  columns 3
  block:toolbar["Toolbar (tools, page tabs, settings)"]:3
  end
  block:editor["Document Editor<br/>(rich text)"]:1
  end
  block:canvas["Canvas Area"]:1
  end
  block:panels["Property Panel<br/>Layer Panel"]:1
  end
  block:status["Status Bar (coordinates, zoom, shape count)"]:3
  end
```
...
See the full list in [Keyboard Shortcuts](/diagrammer/reference/keyboard-shortcuts/).

## Next Steps

Now that you've created your first diagram, explore:

- [Canvas & Tools](/diagrammer/features/canvas-tools/) - Deep dive into all tools
- [Shape Libraries](/diagrammer/features/shape-libraries/) - Use flowchart, UML, and ERD shapes
- [Collaboration](/diagrammer/features/collaboration/) - Work with your team in real-time
